...
title: JoBase template in depth
...
It can be a somewhat daunting task trying to get a handle on _JoBase_ internals, and particularly various points of usage.

This page outlines and clarifies some of the techniques found in _JoBase_. Use it as a guide for further modification, or an instructional for techniques to use in your own endeavors.

## Styling

As the name suggests, _JoBase_ relies on _TailWind_ for styling. Furthermore, _JoBase_ defines custom low level style settings which are incorporated into _TailWind_ seamlessly, and which provides consistency for higher level styling constructs, as well as enabling dark mode.
...
<DListItem dt="layouts/Layout.astro">
  This layout is used for all of the pages rendered by _JoBase_. The contents of _tailwind.css_ and
...
Now, we get to the techniques used in _JoBase_, we'll use the _pages/index.astro_ file to illustrate.
